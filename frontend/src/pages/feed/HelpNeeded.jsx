...
export default function FeedHelpNeeded() {
  ...
  const fetchFeed = useCallback(
    async (cursor = null) => {
      try {
        const response = await feedService.helpNeededFeed(cursor, pageSize);
        ...
        toast.error("Failed to load help needed feed");
      }
    },
    [pageSize],
  );
      ...
      <div className="mx-auto max-w-2xl px-4 py-8">
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground mt-4">No help requests yet</p>
            <p className="text-sm text-muted-foreground">
              When someone needs help, it will appear here
            </p>
          </CardContent>
        </Card>
      </div>
    ...
    <div className="mx-auto max-w-2xl space-y-4 px-4 py-8">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Help Needed</h1>
        </div>
      ...
      {!hasNext && items.length > 0 && (
        <p className="text-center text-sm text-muted-foreground py-4">
          You've reached the end of help requests
        </p>
      )}
    </div>
  );
